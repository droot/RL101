...
### View B: Split Infrastructure & Weight Sync
In real-world applications, Training Infrastructure and Inference Infrastructure are separated physically. The active model generating data ($M_i$) runs on inference hardware (GPUs/TPUs optimized for low latency), while the model being updated runs on training hardware (optimized for high throughput).

This physical separation introduces a critical **Weight Sync** step.

![Split Infra Training Loop](rl_training_loop_detailed.svg)

---

## 3. Throughput & Bottlenecks

As a user post-training a model, the overall turnaround time for training depends on **throughput**—how quickly the system can gather data and run backpropagation. This pipeline involves both Infrastructure bounds and User bounds.

- **Infrastructure Bounds (Compute):** The raw speed of the underlying GPUs/TPUs during both generation (Inference) and backpropagation (Training). This is largely solved by powerful infrastructure providers.
- **User/System Bounds (I/O & Environment):** Data batch availability heavily depends on multi-turn prompt latency, slow tool call responses, environment isolation overhead, and the Grader's execution speed.

The physical separation of inference and training means that Training GPUs often sit idle ("starved" for data) while the Inference side computes trajectories and waits for external environment tools.

### Bottleneck Visualization (Batch Timeline)
![Throughput Bottlenecks](rl_throughput_bottlenecks.svg)

---

## Core Concepts of RL 101
