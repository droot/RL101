...
- **Server Bounds (Compute):** The raw speed of the underlying GPUs/TPUs during both generation (Inference) and backpropagation (Training). This is largely solved by powerful server hardware providers.
...
The physical separation of inference and training means that the Trainer's GPUs often sit idle ("starved" for data) while the Inference Server computes trajectories and waits for external environment tools.
